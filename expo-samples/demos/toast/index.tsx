...
export default () => {
    ...
    const onChangeLoading = () => {
        const id = Toast.loading('我是loading toast\n并且默认永久存在,你可以将duration设置为大于0的值\n或者调用toast.hide使其消失', {
            style: {maxWidth: 300}
        })
        setTimeout(() => {
            Toast.hide(id)
    ...
    return (
        <ScrollView style={{ flex: 1 }}>
            <View style={{ flex: 1, alignItems: 'center', height: 700, justifyContent: 'space-around' }}>
                <Button line onClick={() => {onChangeToast('success')}}>成功提示</Button>
                <Button line onClick={() => {onChangeToast('fail')}}>失败提示</Button>
                <Button line onClick={() => {onChangeToast('warn')}}>提示信息</Button>
                <Button line onClick={() => {onChangeToast('show')}}>默认提示</Button>
                <Button line onClick={onChangeLoading}>Loading</Button>
                <Button line onClick={onChangeMask}>Mask:true</Button>
                <Button line onClick={() => {
                    Toast.show(
                        <>
                ...
                <Button line onClick={onChangePermanent}>永久显示Toast</Button>
