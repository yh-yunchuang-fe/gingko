...
function Grid(props) {
    ...
    const renderItem = (item, index) => {
        ...
        if (renderItem) {
            ...
            const icon = react_1.default.isValidElement(item.icon) ?
                item.icon : (<react_native_1.Image source={{ uri: item.icon }} style={style_1.default.icon}/>);
            return (<react_native_1.View style={style_1.default.gridItem}>
                    {icon}
                    <react_native_1.Text style={style_1.default.text}>{item.text}</react_native_1.Text>
                </react_native_1.View>);
        }
    };
    ...
    for (let i = 0; i < rowCount; i++) {
        ...
        rowsArr.push(<react_native_1.View key={i} style={[style_1.default.gridRow, boxBorderStyle]}>
                {arr}
            </react_native_1.View>);
    }
    return (<react_native_1.View style={[style_1.default.gridContainer, { paddingHorizontal: blankWidth }]}>
            {rowsArr}
        </react_native_1.View>);
}
