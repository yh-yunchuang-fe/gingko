    ...
    constructor() {
        ...
        this.renderIcon = () => {
            ...
            if (value && value.length > 0) {
                return (<react_native_1.TouchableOpacity onPress={this.clear}>
    ...
    render() {
        ...
        return (<react_native_1.View style={[{ flexDirection: 'row' }, style]}>
                <react_native_1.View style={index_1.default.container}>
                    <react_native_1.Text style={index_1.default.label}>{children}</react_native_1.Text>
                    <react_native_1.View style={[index_1.default.input, inputStyle]}>
                        <react_native_1.TextInput {...restProps} {...valueProps} onChange={(event) => this.onChange(event.nativeEvent.text)} onBlur={this.onInputBlur} onFocus={this.onInputFocus} secureTextEntry={type === 'password'} keyboardType={keyboardType} placeholderTextColor='#ddd' style={index_1.default.textInput} underlineColorAndroid='transparent' ref={(ref) => this.inputRef = ref}/>
                        {this.renderIcon()}
                    </react_native_1.View>
                    {error && errorHint ? <react_native_1.Text style={index_1.default.hintText}>{errorHint}</react_native_1.Text> : null}
                </react_native_1.View>
            </react_native_1.View>);
    }
}
