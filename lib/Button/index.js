    ...
    render() {
        ...
        if (icon) {
            if (typeof icon === 'string') {
                iconDom = (react_1.default.createElement(react_native_1.View, { style: { flexDirection: 'row', alignItems: 'center' } },
                    react_1.default.createElement(Icon_1.default, { name: icon, style: style_1.default.iconSty })));
            }
            else if (typeof icon === 'object' && !!icon.name) {
                iconDom = (react_1.default.createElement(react_native_1.View, { style: { flexDirection: 'row', alignItems: 'center' } },
                    react_1.default.createElement(Icon_1.default, { name: icon.name, color: icon.color, size: icon.size, style: [style_1.default.iconSty, icon.style] })));
            }
        }
        let childrenDom;
        if (react_1.default.isValidElement(children)) {
            ...
            childrenDom = react_1.default.createElement(react_native_1.Text, { style: [style_1.default.text, textSty], numberOfLines: 1 }, children);
        }
        return (react_1.default.createElement(react_native_1.TouchableHighlight, Object.assign({ activeOpacity: 1, underlayColor: underlayColor, onPress: (e) => { onClick && onClick(e); }, onPressIn: this.onPressIn, onPressOut: this.onPressOut, onShowUnderlay: this.onShowUnderlay, onHideUnderlay: this.onHideUnderlay, disabled: disabled, style: wrapperSty }, restProps),
            react_1.default.createElement(react_native_1.View, { style: style_1.default.container },
                loading ? react_1.default.createElement(index_1.Indicator, { style: style_1.default.indicator, color: type === 'primary' ? 'white' : 'blue' }) : null,
                iconDom,
                childrenDom)));
    }
}
