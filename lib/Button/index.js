"use strict";
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) if (e.indexOf(p[i]) < 0)
            t[p[i]] = s[p[i]];
    return t;
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
}
Object.defineProperty(exports, "__esModule", { value: true });
const react_1 = __importDefault(require("react"));
const react_native_1 = require("react-native");
const index_1 = require("../index");
const style_1 = __importDefault(require("./style"));
class Button extends react_1.default.Component {
    constructor(props) {
        super(props);
        this.onPressIn = (...args) => {
            this.setState({ pressIn: true });
            if (this.props.onPressIn) {
                this.props.onPressIn(...args);
            }
        };
        this.onPressOut = (...args) => {
            this.setState({ pressIn: false });
            if (this.props.onPressOut) {
                this.props.onPressOut(...args);
            }
        };
        this.onShowUnderlay = (...arg) => {
            this.setState({ touchIt: true });
            if (this.props.onShowUnderlay) {
                this.props.onShowUnderlay(...arg);
            }
        };
        this.onHideUnderlay = (...arg) => {
            this.setState({ touchIt: false });
            if (this.props.onHideUnderlay) {
                this.props.onHideUnderlay(...arg);
            }
        };
        this.state = {
            pressIn: false,
            touchIt: false
        };
    }
    render() {
        const _a = this.props, { size, type, style, disabled, activeStyle, onClick, loading, children } = _a, restProps = __rest(_a, ["size", "type", "style", "disabled", "activeStyle", "onClick", "loading", "children"]);
        ['activeOpacity', 'underlayColor', 'onPress', 'onPressIn',
            'onPressOut', 'onShowUnderlay', 'onHideUnderlay'].forEach((prop) => {
            if (restProps.hasOwnProperty(prop)) {
                delete restProps[prop];
            }
        });
        const wrapperSty = [
            style_1.default.wrapperSty,
            style_1.default[`${size}Sty`],
            style_1.default[`${type}Sty`],
            disabled && style_1.default[`${type}DisabledSty`],
            // this.state.pressIn && activeStyle && btnStyles[`${type}TapSty`],
            activeStyle && this.state.touchIt && activeStyle,
            style
        ];
        const textSty = [
            style_1.default[`${size}Text`],
            style_1.default[`${type}Text`],
            disabled && style_1.default[`${type}DisabledText`],
            this.state.pressIn && style_1.default[`${type}TapText`],
        ];
        const underlayColor = react_native_1.StyleSheet.flatten(style_1.default[activeStyle ? `${type}TapSty` : `${type}Sty`]).backgroundColor;
        return (react_1.default.createElement(react_native_1.TouchableHighlight, Object.assign({ activeOpacity: 1, underlayColor: underlayColor, onPress: (e) => { onClick && onClick(e); }, onPressIn: this.onPressIn, onPressOut: this.onPressOut, onShowUnderlay: this.onShowUnderlay, onHideUnderlay: this.onHideUnderlay, disabled: disabled, style: wrapperSty }, restProps),
            react_1.default.createElement(react_native_1.View, { style: style_1.default.container },
                loading ? react_1.default.createElement(index_1.Indicator, { style: style_1.default.indicator, color: type === 'primary' ? 'white' : 'blue' }) : null,
                react_1.default.createElement(react_native_1.Text, { style: [style_1.default.text, textSty], numberOfLines: 1 }, children))));
    }
}
Button.defaultProps = {
    size: 'large',
    type: 'default',
    disabled: false,
    style: {},
    loading: false,
    activeStyle: {},
    onClick: (_x) => { },
    onPressIn: (_x) => { },
    onPressOut: (_x) => { }
};
exports.default = Button;
//# sourceMappingURL=index.js.map