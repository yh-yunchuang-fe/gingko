...
class GkModal extends react_1.default.Component {
    ...
    render() {
        ...
        if (footer && footer.length) {
            if (footer.length === 1) {
                const { text, type, onPress, style } = footer[0];
                const onPressFn = () => {
                    if (onPress) {
                        onPress();
                    }
                ...
                footerDom = (react_1.default.createElement(react_native_1.View, { style: [style_1.default.btnGroup, style_1.default.singleBtn] },
                    react_1.default.createElement(Button_1.default, { type: newType, style: Object.assign({ width: variables_1.default.modal_single_btn_width }, style), onClick: onPressFn }, text)));
            }
            if (footer.length === 2) {
                const buttons = footer.map((button, idx) => {
                    const { text, type, onPress, style } = button;
                    const onPressFn = () => {
                        if (onPress) {
                            onPress();
                        }
                    ...
                    return (react_1.default.createElement(Button_1.default, { type: type, key: idx, style: Object.assign({ width: variables_1.default.modal_group_btn_width }, style), onClick: onPressFn }, text));
                });
