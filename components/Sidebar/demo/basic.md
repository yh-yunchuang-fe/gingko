...
title:
  zh-CN: 基本
  en-US: Basic
mobile-render: ./sidebar.png
---

```jsx
import React from 'react';
import { View, Text, ScrollView, StyleSheet } from 'react-native';
import { WhiteSpace, SideBar } from 'gingko';
  ...
  const renderDefaultSideBar = (tab: any, isTabActive: boolean) => {
    return (
      <View style={{ height: 20, flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        {isTabActive && <View style={styles.defaultSideBarActive} />}
        <Text style={isTabActive ? styles.activeText : styles.text}>{tab.title}</Text>
      </View>
    );
  };
